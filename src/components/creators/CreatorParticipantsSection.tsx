...
const steps = [
  {
    ...
    preview: (
      <div className="rounded-lg border border-line bg-void/60 p-3 text-left">
        ...
        <p className="mt-2 rounded-md border border-electric/25 bg-electric/10 px-2 py-1 text-center text-[0.72rem] font-semibold text-electric">
          Follow
        </p>
      </div>
    ),
  },
  {
    ...
    preview: (
      <div className="space-y-2 rounded-lg border border-line bg-void/60 p-3 text-left">
        {[
          { label: "BTC", pct: 30 },
          { label: "ETH", pct: 20 },
          { label: "Gold", pct: 15 },
    ...
    preview: (
      <div className="rounded-lg border border-line bg-void/60 p-3 text-left">
        <div className="flex items-center justify-between gap-2">
          ...
          <p className="rounded-full border border-success/35 bg-success/10 px-2 py-0.5 text-[0.65rem] font-semibold text-success">
            Keys retained
          </p>
        </div>
        ...
        <div className="mt-2 h-8 rounded-md border border-electric/30 bg-electric/10" />
      </div>
    ),
  },
] as const;
        ...
        <FadeIn className="mt-10">
          <div className="mx-auto grid max-w-5xl gap-3 md:grid-cols-3">
            {steps.map((step, i) => (
              <article
                key={step.title}
                className="relative flex h-full flex-col rounded-[1.2rem] border border-line bg-void/50 p-4 sm:p-5"
              >
                <p className="display text-[1.25rem] gradient-text">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <h3 className="mt-1.5 display text-[1.2rem] tracking-[-0.02em] text-ink uppercase">
                  {step.title}
                </h3>
                ...
                {i < steps.length - 1 && (
                  <span
                    className="pointer-events-none absolute -right-2 top-10 z-10 hidden text-electric/40 md:block"
                    aria-hidden
                  >
                    →
                  </span>
