"use client";

import { FadeIn } from "@/components/ui/FadeIn";
import { TriggerAction } from "@/components/strategies/TriggerAction";

export function StopLossSection() {
  return (
    <section className="relative border-t border-line bg-deep py-14 md:py-20">
      <div className="section-pad container-max">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center lg:gap-12">
          <FadeIn className="order-2 lg:order-1">
            <div className="rounded-[1.5rem] border border-danger/30 bg-gradient-to-b from-danger/10 to-transparent p-5 sm:p-6">
              <p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-muted-dim">
                Downside protection
              </p>
              <div className="mt-6 space-y-0">
                {[
                  {
                    label: "Entry",
                    detail: "Position open",
                    accent: "border-line bg-void/55",
                  },
                  {
                    label: "Stop Level",
                    detail: "Defined max loss",
                    accent: "border-danger/45 bg-danger/15",
                  },
                  {
                    label: "Reduce / Exit Exposure",
                    detail: "Rule executes",
                    accent: "border-danger/35 bg-danger/10",
                  },
                ].map((step, i) => (
                  <div key={step.label}>
                    <div
                      className={`flex items-center justify-between gap-3 rounded-2xl border px-4 py-4 ${step.accent}`}
                    >
                      <span className="display text-[1.05rem] sm:text-[1.1rem]">
                        {step.label}
                      </span>
                      <span className="text-sm text-muted">{step.detail}</span>
                    </div>
                    {i < 2 && (
                      <div className="flex justify-center py-1.5" aria-hidden>
                        <span className="text-danger/60">→</span>
                      </div>
                    )}
                  </div>
                ))}
              </div>
            </div>
          </FadeIn>

          <FadeIn className="order-1 lg:order-2">
            <h2 className="display text-[clamp(2rem,4.5vw,3.1rem)] uppercase tracking-[-0.02em] text-balance">
              Stop Loss
            </h2>
            <p className="display mt-4 text-[clamp(1.15rem,2.4vw,1.45rem)] text-ink text-balance">
              Protect The Downside Before It Gets Worse.
            </p>
            <p className="mt-5 text-[1.05rem] leading-relaxed text-muted">
              Set the maximum loss you are willing to accept before entering a
              position.
            </p>
            <div className="mt-7">
              <TriggerAction
                trigger="Price reaches your stop level"
                action="Reduce or exit exposure"
                tone="sell"
              />
            </div>
            <div className="mt-6 space-y-2 text-[1.02rem] leading-relaxed text-muted">
              <p>No waiting for a recovery that may never come.</p>
              <p>No emotional decision while the market is falling.</p>
              <p>
                Your downside rule is defined before the position is opened.
              </p>
            </div>
          </FadeIn>
        </div>
      </div>
    </section>
  );
}
